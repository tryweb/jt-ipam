...
# =============================================================================
# docker-compose.offline.yml (→ docker-compose.yml in package) — Generated by build-docker-package.sh
...
#   3. Auto-generates .env with secure random values
#   4. Starts all services via docker-compose.yml
# =============================================================================
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    -d|--directory) INSTALL_DIR="$2"; shift 2; ASK_DIR=false ;;
...
if $ASK_DIR; then
  read -r -p "  Install directory [${INSTALL_DIR}]: " input_dir
...
cp "${PKG_DIR}/docker-compose.yml" "${INSTALL_DIR}/"
...
gen_adminpw(){ openssl rand -base64 18 | tr -d '/+=' | cut -c1-16; }
...
if [[ -f ".env" ]]; then
  ...
  SECRET_KEY=$(gen_hex64)
  ENCRYPTION_KEY=$(gen_b64)
  AUDIT_CHAIN_GENESIS=$(gen_hex64)
  BOOTSTRAP_ADMIN_PASSWORD=$(gen_adminpw)
  ...
  API_PUBLIC_URL="${APP_PUBLIC_URL}/api"
  CORS_ORIGINS="${APP_PUBLIC_URL}"

  cp .env.example .env

  sed -i "s|^APP_PUBLIC_URL=.*|APP_PUBLIC_URL=${APP_PUBLIC_URL}|" .env
  sed -i "s|^API_PUBLIC_URL=.*|API_PUBLIC_URL=${API_PUBLIC_URL}|" .env
  ...
  echo -e "    Password: ${BOOTSTRAP_ADMIN_PASSWORD}"
  echo ""
  echo -e "  ${BOLD}Other secrets saved to .env:${NC}"
  echo -e "    POSTGRES_PASSWORD:   ${POSTGRES_PASSWORD}"
  echo -e "    SECRET_KEY:          ${SECRET_KEY:0:16}..."
  echo -e "    ENCRYPTION_KEY:      ${ENCRYPTION_KEY:0:16}..."
  echo -e "    AUDIT_CHAIN_GENESIS: ${AUDIT_CHAIN_GENESIS:0:16}..."
  echo ""
  echo -e "  ${YELLOW}Save the admin password — it will not be shown again.${NC}"
  echo ""
fi
...
echo ""
read -r -p "  Start the stack now? [Y/n] " start_now
if [[ "${start_now,,}" != "n" && "${start_now,,}" != "no" ]]; then
  ...
  echo ""
  info "You can start later:  cd ${INSTALL_DIR} && docker compose up -d"
  echo ""
fi

echo -e "  ${BOLD}Manage from:${NC}  cd ${INSTALL_DIR}"
echo -e "  ${BOLD}Admin:${NC}       http://localhost:${FRONTEND_PORT}"
echo -e "  ${BOLD}Logs:${NC}        docker compose logs -f backend"
echo -e "  ${BOLD}Stop:${NC}        docker compose down"
echo -e "  ${BOLD}Backup:${NC}      bash scripts/docker-backup.sh"
echo -e "  ${BOLD}Restore:${NC}     bash scripts/docker-restore.sh <timestamp>"
echo ""
INSTALL_EOF
...
Files:
  ...
  docker-compose.yml  — Compose file referencing local images
