"""RDP 連線管理權限：can_use_rdp 各組合（deny-by-default，沿用 can_ssh 能力）。"""
...
from app.services.permission import can_use_rdp
...
async def _ip(db_session, *, rdp_enabled: bool) -> tuple[Subnet, IPAddress]:
    s = Section(name=f"s-{uuid.uuid4().hex[:6]}", strict_mode=False, display_order=0)
    db_session.add(s)
    await db_session.flush()
    sn = Subnet(cidr="10.91.0.0/24", section_id=s.id)
    db_session.add(sn)
    await db_session.flush()
    ip = IPAddress(subnet_id=sn.id, ip="10.91.0.5", state="active", rdp_enabled=rdp_enabled)
    db_session.add(ip)
    await db_session.flush()
    return sn, ip
...
async def test_disabled_blocks_everyone_including_admin(db_session):
    ...
    _sn, ip = await _ip(db_session, rdp_enabled=False)
    assert await can_use_rdp(db_session, user=admin, ip=ip) is False


async def test_admin_can_use_when_enabled(db_session):
    admin = await _user(db_session, admin=True)
    _sn, ip = await _ip(db_session, rdp_enabled=True)
    assert await can_use_rdp(db_session, user=admin, ip=ip) is True


async def test_write_on_subnet_can_use(db_session):
    u = await _user(db_session)
    sn, ip = await _ip(db_session, rdp_enabled=True)
    _grant(db_session, oid=sn.id, user_id=u.id, level="write")
    await db_session.flush()
    assert await can_use_rdp(db_session, user=u, ip=ip) is True
...
async def test_read_only_without_capability_cannot_use(db_session):
    ...
    _grant(db_session, oid=sn.id, user_id=u.id, level="read")
    await db_session.flush()
    assert await can_use_rdp(db_session, user=u, ip=ip) is False
